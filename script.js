let canvas = document.getElementsByTagName("canvas")[0],
	...
	buttonAddDataPointsManually = document.getElementById(
		"add-data-points-manually"
	),
	buttonAddDataPointsRandomly = document.getElementById(
		"add-data-points-randomly"
	),
	buttonRemoveAllDataPoints = document.getElementById(
		"remove-all-data-points"
	),
	buttonAddCentroidsManually = document.getElementById(
		"add-centroids-manually"
	),
	buttonAddCentroidsRandomly = document.getElementById(
		"add-centroids-randomly"
	),
	...
	buttonUpdateCentroidsPositions = document.getElementById(
		"update-centroids-positions"
	),
	buttonRunStepsInLoop = document.getElementById("run-steps-in-loop"),
	inputAddDataPointsRandomlyCount = document.getElementById(
		"add-data-points-randomly-count"
	),
	inputAddCentroidsRandomlyCount = document.getElementById(
		"add-centroids-randomly-count"
	),
	inputRunStepsInLoopMilliseconds = document.getElementById(
		"run-steps-in-loop-milliseconds"
	),
	selectDistanceFunction = document.getElementById("distance-function");

canvas.addEventListener(
	"click",
	(e) => addNewPoint(getPointClickedOnCanvas(e)),
	false
);

buttonAddDataPointsManually.addEventListener(
	"click",
	toggleAddingDataPointsManually,
	false
);
buttonAddDataPointsRandomly.addEventListener(
	"click",
	() => addDataPointsRandomly(+inputAddDataPointsRandomlyCount.value),
	false
);
buttonRemoveAllDataPoints.addEventListener("click", removeAllDataPoints, false);

buttonAddCentroidsManually.addEventListener(
	"click",
	toggleAddingCentroidsManually,
	false
);
buttonAddCentroidsRandomly.addEventListener(
	"click",
...
buttonUpdateCentroidsPositions.addEventListener(
	"click",
...
inputAddDataPointsRandomlyCount.addEventListener("keyup", (e) =>
	ifEnterThenCall(e, () => buttonAddDataPointsRandomly.click())
);
inputAddCentroidsRandomlyCount.addEventListener("keyup", (e) =>
	ifEnterThenCall(e, () => buttonAddCentroidsRandomly.click())
);
inputRunStepsInLoopMilliseconds.addEventListener("keyup", (e) =>
	ifEnterThenCall(e, restartLoop)
);

fillDistanceFunctionSelect();
changeDistanceFunction();
selectDistanceFunction.addEventListener(
	"change",
	changeDistanceFunction,
	false
);
...
let dataPoints = [],
	...
	steps = [reassignDataPoints, updateCentroidsPositions],
	currentStep,
	nextAfter,
	timeout,
	loopRunning = false;
...
function getPointClickedOnCanvas(e) {
	...
	return [e.clientX - canvasRect.left - 1, e.clientY - canvasRect.top - 1];
}
...
function updateCentroidsPositions() {
	centroids.map((centroid, centroidIndex) => {
		let assignedPoints = dataPoints.filter(
				(_, pointIndex) =>
					dataPointsAssignedCentroids[pointIndex] == centroidIndex
			),
			sumX = 0,
			sumY = 0;
